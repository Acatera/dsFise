...
uses
  ...
  StrUtils, StForm, sEdit, sCheckBox, sComboBox, SLabel,
...
function TGUI_FisaCont.Start(Options: integer; FRMFile: string): integer;
begin
...
procedure TGUI_FisaCont.MyEditBtnClick(Sender: TObject);
begin
  EditBtnClick(Sender);
...
procedure TGUI_FisaCont.Btn_OkClick(Sender: TObject);
const
...
begin
  SSTatie := '1';
  DoSQL(q,'DROP TABLE IF EXISTS TMP_NOTE' + SStatie);
  DoSQL(q,
  ...
  if not FileExists(ChangeFileExt(NUME_COD, '.LST')) then
    begin
      ShowMessage('Fisierul ' + ChangeFileExt(NUME_COD, '.LST') + ' nu exista !');
      Exit;
    end;
...
procedure TGUI_FisaCont.Btn_RenuntaClick(Sender: TObject);
begin
  ModalResult := mrCancel;
end;

procedure TGUI_FisaCont.cbTipKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_END then
    Exit;
...
procedure TGUI_FisaCont.ControlKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
...
procedure TGUI_FisaCont.EPerioadaKeyPress(Sender: TObject; var Key: Char);
begin
...
function TGUI_FisaCont.CheckDate(Sender: TObject): boolean;
begin
  Result := True;
...
procedure TGUI_FisaCont.EExit(Sender: TObject);
begin
...
procedure TGUI_FisaCont.EContEnter(Sender: TObject);
//var
//  FPLC : TFActGeneral;
begin
...
procedure TGUI_FisaCont.EEnter(Sender: TObject);
begin
  EnterExit(Sender, True);
end;

procedure TGUI_FisaCont.EPerioadaKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_End then
    Btn_OkClick(Self);
...
procedure TGUI_FisaCont.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  //MySQL.SQLExec('DROP TABLE IF EXISTS TMP_NOTE' + SStatie);
end;

procedure TGUI_FisaCont.FormCreate(Sender: TObject);
begin
  q.DatabaseName := MainForm.db.DatabaseName;
end;

procedure TGUI_FisaCont.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_END then
    begin
...
procedure TGUI_FisaCont.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #27 then ModalResult := mrCancel;
end;

procedure TGUI_FisaCont.FormShow(Sender: TObject);
var
  SLunaAn: string;
begin
  SLunaAn := '0210';
  LTitlu.Width := ClientWidth;
  DoSQL(q,
    'SELECT ' +
      'CONT,' +
      'IF(TIPC="NU", "L4",' +
  ...
  while not q.Eof do begin
    if q.FieldByName('LISTA').AsString = 'L1' then
      Lista1 := Lista1 + '","' + q.FieldByName('CONT').AsString;
    if q.FieldByName('LISTA').AsString = 'L2' then
      Lista2 := Lista2 + '","' + q.FieldByName('CONT').AsString;
    if q.FieldByName('LISTA').AsString = 'L4' then
      Lista1 := Lista4 + '","' + q.FieldByName('CONT').AsString;
    q.Next;  
  end;
  Delete(Lista1,1,2);
  Lista1 := Lista1 + '"';
...
procedure TGUI_FisaCont.EnterExit(Sender: TObject; IsEnter: boolean);
var
  T: TComponent;
begin
  T := TComponent(Sender);
